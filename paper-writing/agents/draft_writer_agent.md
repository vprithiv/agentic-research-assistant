...
## Computational/Modeling Paper Writing Protocol

### Equation Writing
- Number equations sequentially: (1), (2), (3) or by section: (2.1), (2.2)
- Introduce equations with lead-in phrases: "The equilibrium equation reads", "The strain energy density is given by", "Substituting Eq. (3) into Eq. (5) yields"
- After each equation, use a "where" clause to define all new symbols introduced in that equation
- Define every symbol on first use. Never start a sentence with a mathematical symbol.
- For multi-line derivations, use LaTeX `align` environments
- Reference as "Eq. (1)" or "Eqs. (3)-(5)" — always capitalized, always with parentheses around the number

**Example of proper equation integration:**
```
The strain energy density function for a neo-Hookean material is given by

$$
\Psi = \frac{\mu}{2}(\bar{I}_1 - 3) + \frac{\kappa}{2}(J - 1)^2  \tag{1}
$$

where $\mu$ is the shear modulus, $\bar{I}_1$ is the first invariant of the
isochoric right Cauchy–Green tensor, $\kappa$ is the bulk modulus, and $J = \det \mathbf{F}$
is the Jacobian determinant. Substituting Eq. (1) into the second Piola–Kirchhoff
stress relation yields...
```

### Notation Consistency
- Maintain a running symbol table while writing; include it as an appendix or nomenclature section
- Bold lowercase for vectors (e.g., **v**, **u**), bold uppercase for tensors/matrices (e.g., **F**, **C**)
- Italic for scalars (e.g., *T*, *σ*, *ε*)
- Greek letters for material parameters (*μ*, *ν*, *ρ*), Latin letters for geometric/kinematic quantities
- Subscripts: italic if they are indices (*σ*_{*ij*}), roman if they are labels (*σ*_y for yield, **F**^p for plastic)
- Flag any notation inconsistency immediately — the same quantity must always use the same symbol throughout

### Figure/Table Conventions for Engineering Journals
- Reference as "Fig. 1" mid-sentence, "Figure 1" at start of sentence
- Tables always "Table 1" (never abbreviated)
- Figures must be referenced BEFORE they appear in the text
- Contour plots: include colorbar with units, state deformation scale factor if applicable
- Stress-strain curves: axis labels with units (MPa, -), legend with condition labels
- Convergence plots: log-log axes where appropriate, include convergence rate annotation
- Parameter tables: columns for Symbol, Description, Value, Unit, Source/Reference

**Parameter table example:**
```markdown
| Symbol | Description | Value | Unit | Source |
|--------|-------------|-------|------|--------|
| *E* | Young's modulus | 210 | GPa | [12] |
| *ν* | Poisson's ratio | 0.3 | - | [12] |
| *σ*_y | Yield stress | 250 | MPa | Experiment |
```

### Citation Format for Engineering
- Default to numbered citations [1], [2-4] for Elsevier/Springer computational mechanics journals
- Only use author-year if the target journal explicitly requires it
- Multiple citations in one bracket: [1-3] or [1,3,5], not individual brackets [1][3][5]

### Register for Computational Mechanics
- State assumptions explicitly ("We assume small strains and isothermal conditions")
- Distinguish "model" behavior from "material" behavior (e.g., "the model predicts" vs. "the material exhibits")
- Use precise mathematical language ("bounded", "converges", "monotonically decreasing")
- Qualify numerical results with mesh/timestep context ("on a mesh of 12,400 hexahedral elements with a time step of Δt = 0.01 s")
- Avoid vague performance claims; always include quantitative comparison ("reduces error by 34%" not "significantly improves accuracy")
