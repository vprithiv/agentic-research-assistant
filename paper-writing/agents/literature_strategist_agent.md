...
If a Working Set (Schema 11) is available from a prior `literature-review` run:
1. **Accept** all `included` entries as-is — skip Phase A for these sources
2. **Re-screen** any `pending_fulltext` entries through Phase B
3. **Append** new searches to the existing working set's `query_history` (do not start fresh)
4. **Increment** the working set `version`
5. Use the working set's `saturation_status` and `tags` to identify gaps requiring additional search

If no prior Working Set exists, create a new one at `state/working_set.json` during Phase A.

### Phase A: Compressed Screening (Title/Abstract → compressed entries only)
- For each candidate, extract ONLY: ID, title_short (max 15 words), TL;DR (1 sentence, max 30 words), year, DOI, tags, quality_tier
- Screen compressed entries against inclusion criteria
- Tag status: `included` / `excluded` / `pending_fulltext`
- Write compressed entries to the Working Set
- **Do NOT generate full annotations in Phase A**
- Target: narrow to 30-50 candidates at ~35 tokens/source

### Phase B: Full-Text Assessment (included + pending sources only)
- Load abstracts and key sections ONE SOURCE AT A TIME
- Generate full annotation (Type, Method, Key Findings, Relevance, Quality, Potential Use)
- Write expanded entry to `state/working_set_expanded/{id}.json`
- Release source text before loading next source
- Target: 15-30 final sources (varies by paper type)

**Token savings:** ~37% vs. annotating all candidates. See `shared/token_architecture.md` Pattern C.
...
The literature strategist produces TWO outputs:

### Output 1: Updated Working Set (Schema 11)

Written to `state/working_set.json` (updated in-place if inheriting from literature-review, or created fresh). Contains all compressed entries with query history and saturation status. This is the primary machine-readable artifact for downstream agents.

### Output 2: Literature Search Report (human-readable)
...
**Inherited from literature-review**: [Yes/No — if Yes, list count of inherited sources]

### Screening Results
- Inherited sources (from literature-review): [N] (skipped Phase A)
- New candidates identified: [N]
- After compressed screening (Phase A): [N] included, [N] excluded, [N] pending
- After full-text assessment (Phase B): [N] final
...
Layer 1: Boolean Search (keyword search)
  ...
  PROCESS:
    ...
    2. List synonyms and alternative terms for each concept
...
| Language preference not specified | Default to English |
...
| `literature-review` (Handoff) | Working Set + Annotated Bibliography | Working Set (Schema 11) at `state/working_set.json` + expanded entries at `state/working_set_expanded/*.json` |
...
- **Handoff receiving rules**: Bibliography received from literature-review goes directly to Phase B (full-text assessment), skipping Phase A
