...
This document records the failure scenarios that the paper-writing skill may encounter at each stage, their trigger conditions, and handling strategies. All agents should refer to this guide when they detect a failure scenario.
...
|---|---------|---------|--------|---------|
| F1 | Insufficient research foundation | Plan mode Step 0 finds no RQ / no data | High | Recommend running `literature-review` first |
...
| F6 | Poor abstract quality | Abstract missing components or out of word range | Medium | Re-run abstract_agent |
...
| F9 | Incomplete handoff materials | From literature-review but missing key materials | Low | List missing items, suggest supplementing or re-running |
...
3. Recommend using literature-review (socratic mode)
4. Explain that they can come back to continue after literature-review is completed
...
I recommend you first use the literature-review tool to:
...
**Detection Indicators**:
- Abstract missing one or more of the 5 required components
- Word count outside 150-300 word range
- Keywords do not reflect the paper's core concepts
- Abstract contains claims not supported by the paper

**Handling Process**:
```
1. Check abstract against 5-component structure (Background, Purpose, Method, Findings, Implications)
2. Verify word count is within range
3. Rewrite based on the actual paper content
4. Ensure abstract is self-contained and accurately represents the paper
```
...
**Trigger Timing**: intake_agent detects literature-review materials but they are incomplete
...
3. Recommend:
   a. Return to literature-review to complete the missing parts
   b. Or supplement within paper-writing (add Phase 0 interview questions)
```
...
**How to Resume**: Bring this record and restart paper-writing; can continue from Phase {N}
```
...
```
F1 (Insufficient research foundation) → Recommend literature-review → May encounter F9 (incomplete materials) upon return
...
| F6 | abstract_agent writes based on the paper content |
