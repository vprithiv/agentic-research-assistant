...
**Not Applicable When**:
- User has no idea about research direction (→ use `literature-review` first)
...
- Just received materials from literature-review and needs to transform them into a paper plan
...
- Chapter Plan → peer-review (review the plan)
...
- Need an in-depth research investigation (→ literature-review)
...
## Paths from literature-review

```
literature-review completed
  │
  ├── literature-review (full mode) outputs:
  │   RQ Brief + Methodology Blueprint + Annotated Bibliography + Synthesis Report
  │   │
  │   ├── Want to write the paper directly ──→ paper-writing (full mode)
  │   │   intake_agent auto-detects materials, skips redundant questions
  │   │
  │   └── Want to plan before writing ──→ paper-writing (plan mode)
  │       socratic_mentor leverages existing materials to accelerate guidance
  │
  └── literature-review (socratic mode) outputs:
      INSIGHT Collection + Synthesis Report
      │
      ├── INSIGHTs are sufficiently clear ──→ paper-writing (full mode)
      │
      └── Need more guidance ──→ paper-writing (plan mode)
          socratic_mentor continues deepening from INSIGHTs
```

## Connecting to peer-review

```
paper-writing completed
  │
  ├── full mode produces complete paper ──→ peer-review (full / guided)
  │   Complete peer review + revision suggestions
  │
  ├── plan mode produces Chapter Plan ──→ peer-review (guided)
  │   Review the plan's feasibility and completeness
  │
  └── reviewer feedback ──→ paper-writing (revision mode)
...
| "Help me search for literature" | lit-review | First confirm: Is it a literature review for a paper or a research investigation? | May need literature-review |
| "I have literature-review results, help me write a paper" | full (skip Phase 0 directly) | full (but intake needs to detect handoff) | Materials need to be properly imported |
...
| literature-review results | Complete paper | full mode (auto-handoff) |
| literature-review results | Guided planning | plan mode |
