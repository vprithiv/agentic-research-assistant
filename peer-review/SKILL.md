---
name: peer-review
description: "A structured self-review tool that examines a paper from multiple analytical perspectives. It helps authors identify potential weaknesses BEFORE submitting to a journal. Supports full review, re-review (verification), quick assessment, methodology focus, and Socratic guided modes. Triggers on: review paper, peer review, manuscript review, referee report, review my paper, critique paper, structured self-review, editorial review."
metadata:
  version: "1.4"
...
A structured self-review tool that examines a paper from multiple analytical perspectives. It helps authors identify potential weaknesses BEFORE submitting to a journal. Automatically identifies the paper's field, dynamically configures 5 analytical perspectives (Editor-in-Chief + 3 reviewers + Devil's Advocate) who examine from four non-overlapping angles — methodology, domain expertise, cross-disciplinary viewpoints, and core argument challenges — ultimately producing a structured Editorial Decision and Revision Roadmap.

## What This Tool IS / IS NOT

### What This Tool IS
- A systematic pre-submission quality check from 5 analytical perspectives
- A way to catch structural, methodological, and argumentative issues early
- A revision planning tool that produces prioritized improvement roadmaps

### What This Tool IS NOT
- A substitute for genuine peer review by independent human domain experts
- A predictor of how a journal will evaluate your paper
- An independent multi-reviewer assessment — all perspectives are generated by the same AI model in sequence; there is no true reviewer independence

### Mandatory Recommendation
After using this tool, submit your paper for review by:
1. At least one human domain expert (advisor, colleague, or collaborator)
2. The target journal's actual peer review process

AI-assisted self-review is a complement to, not a replacement for, human review.

## Epistemic Boundary Protocol

Every analytical perspective agent MUST classify each assessment using these confidence levels:

### CONFIDENT
Standard academic assessment where AI has strong capabilities. Examples: argument structure, citation completeness, writing quality, internal consistency, formatting compliance.

No special prefix needed — state the assessment directly.

### PARTIALLY CONFIDENT
Requires domain-specific knowledge that may exceed AI capabilities. Prefix assessments with:

> "Based on general methodological principles (human domain expert verification recommended):"

Examples: appropriateness of a specific statistical method for the data type, whether a theoretical framework is current in the field, whether effect sizes are typical for the domain.

### LOW CONFIDENCE
Deep specialist knowledge required. State:

> "This aspect requires evaluation by a specialist in [field]. I can identify structural issues but cannot assess technical correctness."

Examples: correctness of novel mathematical proofs, validity of domain-specific experimental protocols, whether findings contradict unpublished/recent work in a narrow subfield.

**Rule**: Every reviewer report dimension MUST carry one of these three labels. The editorial_synthesizer MUST aggregate confidence levels — if multiple dimensions are LOW CONFIDENCE, this MUST be prominently flagged in the Editorial Decision.
...
3. Expanded analysis team from 4 to 5 members
...
2. Dynamically configures the specific identities and expertise of 5 analytical perspectives
3. 5-perspective structured analysis reports (each from a different perspective)
...
**English**: review paper, peer review, manuscript review, referee report, review my paper, critique paper, structured self-review, editorial review

### Non-Trigger Scenarios

| Scenario | Skill to Use |
|----------|-------------|
| Need to write a paper (not review) | `paper-writing` |
| Need in-depth investigation of a research topic | `literature-review` |
| Need to revise a paper (already have review comments) | `paper-writing` (revision mode) |
...
| # | Agent | Role | Phase |
|---|-------|------|-------|
| 1 | `field_analyst_agent` | Analyzes the paper's field, dynamically configures 5 analytical perspective identities | Phase 0 |
| 2 | `eic_agent` | Journal Editor-in-Chief — journal fit, originality, overall quality | Phase 1 |
| 3 | `methodology_reviewer_agent` | Perspective 1 — research design, statistical validity, reproducibility | Phase 1 |
| 4 | `domain_reviewer_agent` | Perspective 2 — literature coverage, theoretical framework, domain contribution | Phase 1 |
| 5 | `perspective_reviewer_agent` | Perspective 3 — cross-disciplinary connections, practical impact, challenging fundamental assumptions | Phase 1 |
...
=== Phase 0: FIELD ANALYSIS & PERSONA CONFIGURATION ===
     ...
     +-> [field_analyst_agent] -> Reviewer Configuration Card (x5)
         ...
         - Dynamically generates specific identities for 5 analytical perspectives:
           * EIC: Which journal's editor, area of expertise, review preferences
           * Perspective 1 (Methodology): Methodological expertise, what they particularly focus on
           * Perspective 2 (Domain): Domain expertise, research interests
           * Perspective 3 (Cross-disciplinary): Cross-disciplinary angle, what unique perspective they bring
...
=== Phase 1: PARALLEL MULTI-PERSPECTIVE REVIEW ===
     ...
     |   - Does not go deep into methodology (that's Perspective 1's job)
...
=== Phase 2: EDITORIAL SYNTHESIS & DECISION ===
     ...
     +-> [editorial_synthesizer_agent] -> Editorial Decision Package
         ...
         - Revision Roadmap (prioritized, can be directly input to paper-writing revision mode)
     |
...
2. **Phase 1**: 5 analytical perspectives review from non-overlapping angles, without cross-referencing each other
...
4. **Methodology focus**: When author is ready, introduce Perspective 1's methodology perspective
5. **Domain perspective**: Introduce Perspective 2's domain expertise perspective
6. **Cross-disciplinary challenge**: Introduce Perspective 3's unique perspective
...
### Mandatory Human Review Footer

**EVERY** editorial decision output from `editorial_synthesizer_agent` MUST end with the following footer. This is non-negotiable and cannot be omitted regardless of mode or context:

```
---
**Important**: This self-review was generated by an AI assistant analyzing the paper from multiple structured perspectives. All analytical perspectives come from the same AI model (no true reviewer independence). Domain-specific expertise may be shallow or outdated.

**Recommended next step**: Share this paper with at least one human domain expert before submitting to a journal.
---
```
...
```
literature-review --> paper-writing --> [integrity check] --> peer-review --> paper-writing (revision) --> peer-review (re-review) --> [final integrity] --> finalize
...
|----------------------|-------------|
| **Upstream: paper-writing -> reviewer** | Receives the complete paper output from `paper-writing` full mode, directly enters Phase 0 |
| **Upstream: integrity check -> reviewer** | In the Pipeline, the paper must pass integrity check before entering reviewer |
| **Downstream: reviewer -> paper-writing** | The Revision Roadmap format can be directly used as reviewer feedback input for `paper-writing` revision mode |
...
Step 1: literature-review -> Research report
Step 2: paper-writing -> Paper first draft
Step 3: integrity check -> systematic verification of references/data
Step 4: peer-review (full) -> 5 review reports + Revision Roadmap
Step 5: paper-writing (revision) -> Revised manuscript
Step 6: peer-review (re-review) -> Verification review
Step 7: (if needed) paper-writing (revision) -> Second revised manuscript
Step 8: integrity check (final) -> Final systematic verification
Step 9: paper-writing (format-convert) -> Final paper
```
...
All dimension scores (0-100) MUST follow this calibration anchoring:

| Score Range | Meaning | Guidance |
|-------------|---------|----------|
| 90-100 | Would be accepted at a top-5 journal with minimal revision | This score should be **RARE** — reserve for genuinely exceptional work |
| 75-89 | Minor Revision at a Q1 journal | Solid work with identifiable but manageable improvements needed |
| 60-74 | Major Revision — fixable but needs significant work | Clear structural, methodological, or argumentative gaps |
| 45-59 | Fundamental issues that must be resolved before submission | Serious concerns about validity, framing, or contribution |
| <45 | Not ready for submission | Core reconceptualization needed |

### Anti-Inflation Rules

- **Default stance**: Err toward the LOWER end of each range. Human reviewers will be MORE critical than this tool.
- **First-draft warning**: If ALL dimensions score >80 on a first draft, display the following warning prominently in the editorial decision:

> **Calibration Warning**: Unusually high scores for a first draft. Consider whether this assessment is calibrated conservatively enough. AI-generated reviews tend toward score inflation. Seek human expert review to validate these scores.

- **Comparative anchor**: A score of 85+ should correspond to work that the reviewer believes could appear in a top-quartile journal in the field with only minor polishing.
...
| **Epistemic boundary marking** | **Every reviewer must mark confidence level (CONFIDENT / PARTIALLY CONFIDENT / LOW CONFIDENCE) per dimension** |
| **Human review recommendation** | **Every output must include recommendation for human expert review** |

---

## Output Language

English. Academic terms remain in English.
...
| Skill | Relationship |
|-------|-------------|
| `paper-writing` | Upstream (provides paper) + Downstream (receives revision roadmap) |
| `literature-review` | Upstream (provides research foundation) |
| `tw-hei-intelligence` | Auxiliary (verifies higher education data accuracy) |
| `full-workflow` | Orchestrated by (Stage 3 + Stage 3') |
...
| Dependent Skills | paper-writing v1.0+ (upstream/downstream integration) |
| Role | Multi-perspective academic paper structured self-review tool |
...
| 1.1 | 2026-02 | Added Devil's Advocate Reviewer (7th agent), added re-review mode, expanded analysis team from 4 to 5 |
