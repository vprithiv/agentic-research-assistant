...
### Computational Science Calibration

For computational / simulation papers, the standard Evidence Sufficiency and Methodological Rigor descriptors should be re-interpreted as follows:

**Evidence Sufficiency (computational papers)**

| Score Range | Descriptor | Behavioral Indicators |
|------------|------------|----------------------|
| 90-100 | Exceptional | Mesh convergence study (h- and/or p-refinement) with quantified error norms + validation against multiple independent experiments + parameter sensitivity analysis + benchmark comparisons |
| 75-89 | Strong | Convergence study shown with quantified error + at least one benchmark or experimental validation; sensitivity analysis present but limited |
| 60-74 | Adequate | Some convergence evidence presented (e.g., two mesh levels) but not systematic; limited or qualitative comparison with experiments |
| < 60 | Insufficient | No convergence study; results presented on a single mesh without justification; no experimental validation |

**Optional Dimension: Computational Rigor / Reproducibility**

| Score Range | Descriptor | Behavioral Indicators |
|------------|------------|----------------------|
| 90-100 | Exceptional | Full V&V protocol documented (verification via MMS/patch tests, validation via quantified experimental comparison); code and input files publicly available or archived; all material parameters, boundary conditions, and solver settings fully documented; results independently reproducible |
| 75-89 | Strong | V&V partially addressed (verification OR validation present); parameter documentation mostly complete; code availability stated but not archived; reproducibility feasible with moderate effort |
| 60-74 | Adequate | Some verification or validation present but incomplete; key parameters documented but solver settings or boundary conditions partially missing; no code availability |
| < 60 | Insufficient | No V&V protocol; parameter documentation inadequate; results not reproducible from the information provided |
