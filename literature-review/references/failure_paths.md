...
This document lists all failure scenarios that may be encountered across all modes of the literature-review skill, along with their detection conditions, user notification messages, handling steps, and recovery paths. The purpose is to ensure every failure scenario has a clear handling strategy, preventing users from reaching a dead end.
...
| F8 | Limited English-language literature available | full, lit-review | Medium | Switch search strategy |
...
### F8: Limited English-Language Literature Available

**Affected Modes**: `full` (Phase 2), `lit-review`
**Severity**: Medium

**Trigger Conditions**:
- English academic database searches (Web of Science, Scopus, PubMed, etc.) yield few or no results
- The topic is strongly localized (e.g., country-specific policy, regulations, institutional systems)

**User Notification Message**:
> English-language academic literature on this topic is very limited. The topic appears to be strongly localized. I will adjust the search strategy to include grey literature, policy reports, and working papers, and consider broadening the geographic scope.

**Handling Steps**:
1. Expand search to include grey literature, policy reports, working papers, and government publications
2. Broaden geographic scope to find comparable international cases
3. Note the literature availability constraints in the report
4. If the user needs to publish internationally → suggest adjusting RQ to add a comparative perspective

**Recovery Paths**:
- Grey literature and policy reports provide sufficient coverage → continue workflow with source type annotations
