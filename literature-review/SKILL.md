...
A structured research assistance tool — a domain-adaptive 13-agent team that helps organize and synthesize academic research on any topic.

## What This Tool IS / IS NOT

**IS:**
- A systematic literature search and synthesis assistant
- A structured framework for organizing research thinking
- A quality-checking tool for academic reports

**IS NOT:**
- A substitute for domain expertise
- A replacement for reading primary sources
- A guarantee that findings are correct or complete
- An original researcher

**v2.4** adds writing quality improvements to the report compiler:
- **Style Profile consumption** (optional) — If a Style Profile is available from paper-writing intake, the report compiler applies it as a soft guide for the Executive Summary and Synthesis sections. Discipline conventions and report objectivity take priority.
- **Writing Quality Check** — The report compiler runs a writing quality checklist before finalizing: flags AI-typical overused terms, checks sentence/paragraph length variation, removes throat-clearing openers. See `paper-writing/references/writing_quality_check.md`.
...
|----------|-------------|
| Writing a paper (not researching) | `paper-writing` |
...
| # | Agent | Role | Phase | Planned Merge |
|---|-------|------|-------|---------------|
| 1 | `research_question_agent` | Transforms vague topics into precise, FINER-scored research questions with scope boundaries | Phase 1, Socratic Layer 1 | Planned merge with `research_architect_agent` into `research_scoping_agent` |
| 2 | `research_architect_agent` | Designs methodology blueprint: paradigm, method, data strategy, analytical framework, validity criteria | Phase 1 | Planned merge with `research_question_agent` into `research_scoping_agent` |
| 3 | `bibliography_agent` | Systematic literature search, source screening, annotated bibliography in APA 7.0 | Phase 2 | Planned merge with `source_verification_agent` into `literature_agent` |
| 4 | `source_verification_agent` | Fact-checking, source grading (evidence hierarchy), predatory journal detection, conflict-of-interest flagging | Phase 2 | Planned merge with `bibliography_agent` into `literature_agent` |
...
| 7 | `editor_in_chief_agent` | Q1 journal editorial review: originality, rigor, evidence sufficiency, verdict (Accept/Revise/Reject) | Phase 5 | Planned merge with `ethics_review_agent` into `editorial_review_agent` |
...
| 9 | `ethics_review_agent` | AI-assisted research ethics, attribution integrity, dual-use screening, fair representation | Phase 5 | Planned merge with `editor_in_chief_agent` into `editorial_review_agent` |
...
| 11 | `risk_of_bias_agent` | Assesses risk of bias using RoB 2 (RCTs) and ROBINS-I (non-randomized); traffic-light visualization | Systematic Review (Phase 2) | Planned merge with `meta_analysis_agent` into `systematic_review_agent` |
| 12 | `meta_analysis_agent` | Designs and executes meta-analysis or narrative synthesis; effect sizes, heterogeneity, GRADE | Systematic Review (Phase 3) | Planned merge with `risk_of_bias_agent` into `systematic_review_agent` |
| 13 | `monitoring_agent` | Post-research literature monitoring: digests, retraction alerts, contradictory findings detection | Optional (post-pipeline) | Demoted to reference template |
...
=== Phase 1: SCOPING (Interactive) ===
     ...
     ** User confirmation before Phase 2 **
     |
     ** Pre-registration recommendation (optional) **
     For systematic reviews: STRONGLY RECOMMEND pre-registration (PROSPERO, OSF)
     For other research: SUGGEST but do not require. Never block progression.
     |
=== Phase 2: INVESTIGATION ===
     ...
     |
     +-> [source_verification_agent] -> Bibliographically Confirmed & Graded Sources
...
=== Layer 5: SIGNIFICANCE & CONTRIBUTION (conclusion) ===
     ...
     +-> Compile all [INSIGHT]s into Research Plan Summary
         Can directly hand off to paper-writing (plan mode)
```
...
=== Phase 2: INVESTIGATION (PRISMA-Compliant Search + RoB) ===
     ...
     |
     |-> [source_verification_agent] -> Bibliographically Confirmed Sources
...
| Limited English-language literature | Primary search returns insufficient results | Expand search strategy, alternative databases |
...
After research is complete, the following materials can be handed off to `paper-writing`:
...
`paper-writing`'s `intake_agent` will automatically detect available materials and skip redundant steps:
...
| `examples/handoff_to_paper.md` | literature-review full mode handoff to paper-writing |
...
Every factual claim in a research report must carry a confidence marker:

| Marker | Meaning |
|--------|---------|
| **[WELL-SUPPORTED]** | Multiple high-quality, convergent sources confirm this claim |
| **[SUPPORTED]** | At least one peer-reviewed source directly supports this claim |
| **[EXPLORATORY]** | Based on limited, indirect, or emerging evidence; treat as hypothesis |
| **[UNASSESSABLE]** | Cannot determine confidence due to insufficient available evidence |

**Section-level confidence**: Every report section receives an overall confidence rating:
- **HIGH** — Core claims are [WELL-SUPPORTED] or [SUPPORTED]; no unresolved contradictions
- **MODERATE** — Mix of [SUPPORTED] and [EXPLORATORY] claims; some contradictions noted
- **LOW** — Predominantly [EXPLORATORY] or [UNASSESSABLE] claims. Triggers: "Human expert review specifically recommended for this section."

**Mandatory uncertainty disclosures** (included in every report):
- WebSearch verification confirms that a source exists and its metadata is consistent; it does NOT confirm that the source's conclusions are correct
- DOI resolution validates bibliographic metadata; it does NOT verify that the cited content supports the claim as used
- This tool cannot guarantee completeness of the literature search; relevant sources may exist that were not found
- Synthesis and interpretation are generated by an AI system and should be validated by domain experts
...
| **Currency Rule** | Default: published within 5 years. Override by domain: CS/AI = 3 years, History/Philosophy = 20 years, Law = depends on jurisdiction changes, Computational Materials Science = foundational methods: no limit; implementations: 10 years; applications: 5 years. Seminal works exempt regardless of age | bibliography_agent, ethics_review_agent |
...
| **Evidence Taxonomy** | Domain-adaptive. Before grading, identify epistemic domain: Biomedical (systematic reviews > RCTs > cohort), Social Sciences-Quant (meta-analyses > large-N surveys > quasi-experiments), Social Sciences-Qual (thick-description ethnographies > grounded theory > focused case studies), Humanities (primary source analysis > peer-reviewed interpretive scholarship > intellectual tradition), CS/Engineering (Computational): validated simulations against multiple experiments > validated simulation against single experiment > verified simulation (convergence shown, unvalidated) > benchmark reproduction (NIST, community challenges) > simulation with documented parameters but no V&V > unverified simulation. Note: code documentation, technical reports (Sandia SAND, INL/EXT), and benchmark databases are legitimate primary sources. Foundational methods papers have no currency limit, Law (statutes/case law > legal scholarship > practitioner commentary). Do NOT apply biomedical hierarchy to non-biomedical research | literature_agent, source_verification_agent |
...
| **Bibliographic Confirmation Scope** | DOI resolution check (metadata-validated, not content-verified) + 50% WebSearch spot-check (confirms existence, not correctness) | source_verification_agent, ethics_review_agent |
| **Verification does NOT confirm** | Source conclusions are correct, source is the best/most relevant, no sources were missed, interpretive accuracy of citations | All agents |
...
```
literature-review + tw-hei-intelligence     -> Evidence-based HEI policy research
literature-review + report-to-website       -> Interactive research report
literature-review + podcast-script-generator -> Research podcast
literature-review + paper-writing          -> Full research-to-publication pipeline
literature-review (socratic) + paper-writing (plan) -> Guided research + paper planning
literature-review (systematic-review) + paper-writing -> PRISMA systematic review paper
```
...
|---------|------|---------|
| 2.4 | 2026-03-27 | Report compiler now consumes optional Style Profile (from paper-writing intake) and runs Writing Quality Check checklist before finalizing reports. Style Profile applied as soft guide for Executive Summary and Synthesis sections; discipline conventions take priority. Writing Quality Check catches overused AI-typical terms, em dash overuse, throat-clearing openers, and monotonous sentence rhythm. See `paper-writing/references/writing_quality_check.md` and `shared/style_calibration_protocol.md` |
