...
- Use `literature-review` (full mode) for comprehensive literature exploration
- Or use `paper-writing` (plan mode) to start planning the paper directly
```
...
- The Research Plan Summary can be handed directly to `paper-writing` (plan mode)
- If the user wants deeper literature exploration, suggest switching to `literature-review` (full mode)
- `paper-writing`'s `intake_agent` will automatically detect an existing Research Plan Summary and skip redundant steps
