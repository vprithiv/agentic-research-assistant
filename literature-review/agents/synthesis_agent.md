...
### Step 0: Working Set Grep — Pre-Synthesis Pattern Discovery

Before loading any full annotations, run targeted Grep searches across the Working Set files to build a cross-reference map. This identifies patterns, clusters, and surprising connections at zero model-token cost.

```
Pre-synthesis pattern discovery:
  1. Grep state/working_set.json for each key concept from the RQ
     → reveals which sources mention which concepts (via TL;DRs and tags)
  2. Grep state/working_set_expanded/*.json for methodology terms:
     e.g., "FEM|FEA|phase-field|XFEM|cohesive zone|molecular dynamics"
  3. Grep for V&V terms: "convergence|mesh independence|validation|benchmark|MMS"
  4. Grep for material/domain terms specific to the RQ:
     e.g., "SiC|PyC|graphite|TRISO|irradiation|creep"
  5. Tabulate: which source IDs mention which terms
  6. Write cross-reference table to state/cross_reference_matrix.md
  7. Identify:
     - Clusters: groups of sources sharing 3+ terms → likely theme nuclei
     - Outliers: sources matching only 1 term → check if peripheral or cross-disciplinary bridges
     - Gaps: key concepts with < 2 matching sources → flag for gap analysis
  8. Load full annotations ONLY for:
     - Sources appearing in unexpected term combinations (cross-cutting insights)
     - Sources at cluster boundaries (bridge papers)
     - All "core" relevance sources
```

**Why:** This avoids reading 20-30 full annotations sequentially just to build the evidence map. The grep pass typically surfaces 60-70% of the thematic structure in under 30 seconds with zero token cost. Load the remaining 30-40% by targeted full-annotation reads.

See `shared/token_architecture.md` Pattern D for details.

### Step 1: Evidence Mapping

Create a Literature Matrix, seeded from the cross-reference table produced in Step 0. (Reference: `templates/literature_matrix_template.md`)
