...
A hybrid verification strategy to catch hallucinated or fabricated references. Uses **batched in-context verification** with the parent process's tool access.

> **Note:** Verification requires WebFetch (DOI resolution) and WebSearch (spot-checks). These tools are only available in the parent conversation context, NOT in `--print` subprocesses. All verification runs in-context using the batched protocol below.

#### Tier 1: Automated DOI Verification (100% coverage) — Batched

Verify all DOIs using the parent process's WebFetch tool.

```
Batched DOI verification:
  1. Read Working Set (state/working_set.json) for all source DOIs
  2. Partition sources into batches of 10
  3. For each batch:
     a. WebFetch each DOI URL (https://doi.org/{doi}) — up to 10 parallel tool calls
     b. Compare returned page title/metadata against expected title, authors, year
     c. Write batch results to state/verification_batch_{N}.jsonl
     d. Release batch content from context (Read-Process-Write-Forget)
  4. Merge all batches into state/verification_log.jsonl
```

- Check: DOI resolves to a real page, title matches, authors match
- Auto-flag: DOI returns 404 or title mismatch > 3 words

#### Tier 2: WebSearch Spot-Check (50% coverage) — Batched

Verify a sample of sources using the parent process's WebSearch tool.

```
Batched WebSearch verification:
  1. Select 50% of sources for spot-check (priority: ALL tier_3/tier_4 first, then sample tier_1/tier_2)
  2. For each batch of 5 sources:
     a. WebSearch for each: "{exact title}" {first author last name} {year}
     b. Verify: source exists, published in claimed venue, year matches
     c. Write results to state/verification_log.jsonl (append to Tier 1 results)
     d. Release search results from context
```
...
#### Batched Verification Benefits

- **Token isolation:** Each batch of 10 is processed and released before loading the next — no accumulated state bloat
- **Budget:** 50 refs in 5 batches × ~2K tokens/batch = ~10K tokens total, fitting within the 10K verification budget
- **Error isolation:** One failed verification does not block the batch; failed items are flagged and retried at end
...
Uses the same terminal verdict taxonomy as pipeline_policies.md P5:

- `VERIFIED`: DOI resolves + metadata matches
- `NOT_FOUND`: 3 search attempts failed → suspected fabrication. CRITICAL, must remove
- `MISMATCH`: Source exists but metadata differs (wrong year, wrong authors, etc.)
- `MANUAL_CHECK_REQUIRED`: Vague reference with no title and no DOI → flag for human review
- `BOOK_NOT_VERIFIED`: Book (ISBN or publisher known) → skipped automated verification
