...
|-----------|------------|
| stage_id | "1", "2", "2.5", "3", "3.5", "4", "3p", "4p", "4.5", "5", "6" |
| status | "pending", "in_progress", "completed", "skipped", "blocked" |
| details | mode, outputs, decision, verdict, and other additional information |

**Rules:**
- Status normally advances (pending -> in_progress -> completed). Regression to in_progress is permitted ONLY via explicit user `redo` command, which increments the version label of the affected stage's output.
- Exception: Stage 2.5 and 4.5 FAIL retries are legal (status remains in_progress)
- Skipped status means the user skipped this stage. Stage 4.5 cannot be skipped. Stage 2.5 can be skipped under Material Passport conditions (see P1 in pipeline_policies.md).
...
- **`working_set`**: Working Set (Schema 11) — persistent queryable literature collection
- **`research_session`**: Research Session State (Schema 12) — cross-phase research context
...
| Stage 2 | None (but Stage 1 output recommended) | RQ Brief, Bibliography, Synthesis, Working Set, Research Session |
...
| Stage 5 | **Integrity Report (Final) — verdict: PASS or PASS_WITH_NOTES** | -- |
...
| **Missing integrity report** | **Mandatory block; Stage 4.5 never skippable. Stage 2.5 skippable only under P1 passport conditions** |
