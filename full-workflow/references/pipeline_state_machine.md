...
This document defines all legal states, transition conditions, transition actions, and exception handling for full-workflow v2.0.
...
|------|----|-------------|--------|
| INIT | Stage 1 | User confirms starting from Stage 1 | Detect mode preference, launch literature-review |
| INIT | Stage 2 | User has research materials, confirms skipping Stage 1 | Detect materials, launch paper-writing |
...
| Stage 2.5 | **checkpoint** | PASS or PASS_WITH_NOTES | Wait for user confirmation |
...
| Stage 4.5 | **checkpoint** | PASS or PASS_WITH_NOTES | Wait for user confirmation |
...
| Stage 2 | Stage 3 | **Cannot skip Stage 2.5 unless Material Passport conditions (P1) are met** |
...
| completed | in_progress | Completed stages cannot restart EXCEPT via user `redo` command (version is incremented, previous output preserved) |
...
| Can skip any stage | Stage 4.5 never skippable; Stage 2.5 skippable only under P1 passport conditions |
